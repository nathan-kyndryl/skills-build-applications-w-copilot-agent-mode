...
const serializeCollection = (documents) => documents.map(normalizeDocument);
const getCollection = async (model, fallback) => {
    if (mongoose.connection.readyState === 1) {
        const documents = await model.find({}).lean();
        return serializeCollection(documents);
    }
    return fallback;
};
const createCollectionItem = async (model, entityName, payload, fallback) => {
    if (mongoose.connection.readyState === 1) {
...
const registerCollectionRoutes = (resource, model, fallback) => {
    ...
    app.post([routePath, slashRoutePath], async (request, response) => {
        ...
        const createdItem = await createCollectionItem(model, resource.replace(/s$/, ''), payload, fallback);
